...
function convertDiv($, $el) {
  const ALL_STYLES = [
    ...
    'color', 'font-size', 'line-height', 'text-align', 'vertical-align',
    'font-family', 'text-decoration', 'font-weight', 'height', 'width',
    'background-color', 'margin', 'display', 'border-collapse', 'border', 'max-width'
  ];
...
function moveTablePaddingStyles($, $root) {
  ...
  $root.find('table').each((_, table) => {
    ...
    const hasRowOrColumn = classAttr.includes('row') || classAttr.includes('column');
    ...
    if (hasRowOrColumn) {
      ...
      if ($tdParent.length) {
        ...
        $tdParent.attr('style', [parentStyle, paddingString].filter(Boolean).join('; '));
      } else {
      ...
      if ($firstTd.length) {
        ...
        $firstTd.attr('style', [tdStyle, paddingString].filter(Boolean).join('; '));
      }
    }
  });
}
function moveTableTextStylesToTd($, $root) {
  const TEXT_STYLE_PROPS = [
    'color', 'font-size', 'line-height', 'text-align', 'vertical-align',
    'font-family', 'text-decoration', 'font-weight', 'height'
  ];
  ...
  $root.find('table').each((_, table) => {
    ...
    if ($firstTd.length) {
      const tdStyle = $firstTd.attr('style') || '';
      const updatedStyle = [tdStyle, textStyles.join('; ')].filter(Boolean).join('; ');
